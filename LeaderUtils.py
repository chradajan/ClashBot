...
class LeaderUtils(commands.Cog):
    ...
    async def export(self, ctx, update_before_export: bool=True, false_logic_only: bool=True):
        ...
        path = db_utils.output_to_csv(false_logic_only)
    ...
    async def export_error(self, ctx, error):
        if isinstance(error, commands.errors.CheckFailure):
            ...
            await ctx.send("Something went wrong. Command should be formatted as:  !export <update_before_export (optional)> <false_logic_only (optional)>")
            raise error
