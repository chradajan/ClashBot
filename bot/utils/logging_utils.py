...
def log_message(msg: str="", **kwargs) -> str:
    ...
    for key, value in kwargs.items():
        if all(hasattr(value, attr) for attr in ['discriminator', 'name', 'display_name']):
            value = f"{value.display_name} - {bot_utils.full_name(value)}"
...
def command_start_log(self, ctx: commands.Context, *args, **kwargs):
    ...
    if self.isEnabledFor(command_start_num):
        msg = log_message(Command=ctx.invoked_with,
                          Initiator=f"{ctx.author.display_name} - {ctx.author},",
                          Channel=ctx.channel,
                          **kwargs)
...
with open("config/logging_config.json") as logging_config_file:
