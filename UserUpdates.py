...
class UserUpdates(commands.Cog):
    ...
    @staticmethod
    async def reset_user(member: discord.Member, remove_from_db: bool):
        if member.bot:
            return

        roles_to_remove = list(roles.NORMAL_ROLES.values())
        roles_to_remove.append(roles.SPECIAL_ROLES["Check Rules"])
    ...
    async def update(self, ctx):
        ...
        if not bot_utils.update_member(ctx.author):
    ...
    async def update_user(self, ctx, member: discord.Member, player_tag: str = None):
        ...
        if not bot_utils.update_member(member, player_tag):
    ...
    async def reset_user(self, ctx, member: discord.Member):
        ...
        await self.reset_user(member, True)
    ...
    async def reset_all_users(ctx, confirmation: str):
        ...
        for member in ctx.guild.members:
            await self.reset_user(member, False)

        await LeaderUtils.send_rules_message(ctx, self.bot.user)

        admin_role = roles.SPECIAL_ROLES["Admin"]
